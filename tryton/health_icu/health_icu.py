...
class ApacheII(ModelSQL, ModelView):
    ...
    wbc = fields.Float(
        'WBC', help="White blood cells x 1000 - if you"
        " want to input 4500 wbc / ml, type in 4.5")
    hematocrit = fields.Float('Hematocrit')
    gcs = fields.Integer(
        'GSC', help='Last Glasgow Coma Scale'
    ...
    @fields.depends(
        'age', 'temperature', 'mean_ap', 'heart_rate',
    ...
    def on_change_with_apache_score(self):
        ...
        if (self.respiratory_rate):
            if ((self.respiratory_rate >= 10 and self.respiratory_rate < 12) or
                (self.respiratory_rate >= 25 and
                    self.respiratory_rate < 35)):
                total = total + 1
        ...
        if (self.serum_potassium):
            if ((self.serum_potassium >= 3 and self.serum_potassium < 3.5) or
                (self.serum_potassium >= 5.5 and
                    self.serum_potassium < 6)):
                total = total + 1
        ...
        if (self.serum_creatinine):
            ...
            if ((self.serum_creatinine < 0.6) or
                (self.serum_creatinine >= 1.5 and
                    self.serum_creatinine < 2)):
...
class MechanicalVentilation(ModelSQL, ModelView):
    ...
    mv_start = fields.DateTime(
        ...
        required=True)
    mv_end = fields.DateTime(
        'To', help="End of Mechanical Ventilation",
        states={
...
class PatientRounding(ModelSQL, ModelView):
    ...
    icu_patient = fields.Boolean(
        'ICU', help='Check this box if this is'
    ...
    left_pupil = fields.Integer(
        'L', help="size in mm of left pupil",
        states=STATES)
    right_pupil = fields.Integer(
        'R', help="size in mm of right pupil",
        states=STATES)
    ...
    sce = fields.Boolean(
        'SCE', help="Subcutaneous Emphysema", states=STATES)
