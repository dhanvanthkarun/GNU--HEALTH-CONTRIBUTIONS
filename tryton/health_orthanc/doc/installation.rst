...
5. Copy the new modules ``health_imaging_orthanc`` into the GNU Health server:
   ::
      cp -r health_imaging_orthanc /home/gnuhealth/gnuhealth/tryton/server/modules/

      ln -s /home/gnuhealth/gnuhealth/tryton/server/modules/health_imaging_orthanc /gnuhealth/tryton/server/trytond-6.0.43/trytond/modules
...
1. Activate the modules for the demo database:
   ::
      /home/gnuhealth/gnuhealth/tryton/server/trytond-6.0.43/bin/trytond-admin -d ghdemo44 -u health_imaging_orthanc --activate-dependencies
...
8. We have created a new widget that allows to select and upload multiple DICOM files in the desktop client and web client. The widget has to be added to the clients.
    ...
    - For the desktop client: Let's assume that your desktop client is installed in ```/home/gnuhealth/health-hmis-client/```. Copy the widget into the   plugin directory of the client:
      ::
         cp -r /home/health_imaging_orthanc/dicombinary /home/gnuhealth/health-hmis-client/gnuhealth/plugins/
