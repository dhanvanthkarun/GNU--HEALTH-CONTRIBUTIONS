...
9.  Connect to the demo database with the client and open the module ``Configuration/Orthanc_Config``. Select ``Add Orthanc Server`` to configure the connection to the Orthanc server.

10.   Watch the video [#f4]_ to see how to use the ``Health Radiology`` module to fetch DICOM studies from the Orthanc server and open medical images.
